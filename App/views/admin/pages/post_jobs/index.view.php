          ...
          <tbody>
            ...
            foreach ($jobs as $row): ?>
              <tr>
                ...
                <td> <?= $row['isActive'] ? "<label class='badge badge-success'>Hiện</label>" : "<label class='badge badge-danger'>Ẩn</label>" ?> </td>
                <?php
                $content = "<td><a href='/admin-panel/post_job/edit/{$row['id']}' class='btn btn-outline-warning'><i class='fa fa-edit'></i></a> </td>";
                echo Template::can('edit_post_jobs', $content);
                ?>

                <?php
                $content = " <form action='/admin-panel/post_job/delete/{$row['id']}' method='POST'>
                              <input type='hidden' name='_method' value='DELETE'>
                              <td> <button type='submit' class='btn btn-outline-danger'><i class='fa fa-trash-o'></i></button> </td>
                            </form>";
                echo Template::can('delete_post_jobs', $content);
                ?>
              </tr>
            <?php endforeach; ?>
          </tbody>
